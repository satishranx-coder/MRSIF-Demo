...
# ====================================================================
# 2. REFERENCE DICTIONARIES (With Realistic GIS Coordinate Offsets)
# ====================================================================
...
# TAB 4: GIS-COMPLIANT FIELD SPATIAL TWIN VECTOR
with step_tabs[3]:
    ...
    # FIXED: Nesting fixed cleanly. autorange moved up directly under zaxis dictionary block
    fig.update_layout(
...
with step_tabs[3]:
    ...
    fig.update_layout(
        margin=dict(l=0, r=0, b=0, t=0), height=550,
        scene=dict(
            xaxis=dict(title=dict(text="UTM Easting (m)", font=dict(color="#94a3b8")), backgroundcolor="#0b0f19", gridcolor="#1e293b", showbackground=True, tickfont=dict(color="#94a3b8")),
            yaxis=dict(title=dict(text="UTM Northing (m)", font=dict(color="#94a3b8")), backgroundcolor="#0b0f19", gridcolor="#1e293b", showbackground=True, tickfont=dict(color="#94a3b8")),
            zaxis=dict(title=dict(text="Depth Profile (m)", autorange="reversed", backgroundcolor="#0b0f19", gridcolor="#1e293b", showbackground=True, titlefont=dict(color="#94a3b8")), tickfont=dict(color="#94a3b8")),
            camera=dict(eye=dict(x=1.4, y=-1.4, z=1.0))
        ),
        paper_bgcolor="#0b0f19", plot_bgcolor="#0b0f19",
        legend=dict(font=dict(color="#94a3b8"), bgcolor="rgba(11,15,25,0.8)")
    ...
    # Structural GeoJSON layer mapping bundle creation
    geojson_data = {
        "type": "FeatureCollection",
        "crs": {"type": "name", "properties": {"name": "urn:ogc:def:crs:EPSG::32639"}},
        "features": geojson_features
