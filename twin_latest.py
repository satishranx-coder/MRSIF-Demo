...
if page == "MISSION":
    ...
    if stage < 9:
        ...
        st.markdown("### Evidence / Data Handover")
        st.write("Mission timeline, navigation status, tooling validation, photogrammetry registration, intervention evidence and completion verification are ready for the handover package.")

# ============================================================
# OPERATIONS COMMAND BAR
# ============================================================
if page == "OPERATIONS":
    cmd1,cmd2,cmd3,cmd4 = st.columns([1,1.25,1,1.5])
    with cmd1:
    ...
    with cmd2:
        if st.button("Advance Mission →", use_container_width=True, disabled=(not stage_clear or stage>=9)):
            ...
            st.rerun()
    with cmd3:
        if st.button("Acceptance Test", use_container_width=True):
            if stage_clear:
    ...
    with cmd4:
        if stage >= 9:
            st.success("Mission accomplished · open CLOSURE.")
        elif stage_clear:
            st.info("Stage accepted · advance when ready.")
        else:
            st.warning("Advance inhibited by active gate.")
