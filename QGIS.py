...
with step_tabs[3]:
    ...
    st.markdown("Dynamic hazard boundaries projected from MRSIF API thresholds onto the seabed.")

    # Assign explicit RGBA tracking boundaries based on metocean conditions
    if metocean_passed:
        hazard_color = [46, 204, 113, 180]    # Safe: Semi-transparent Green
        status_text = "[PASSED] Safe to Mobilize"
    else:
        hazard_color = [231, 76, 60, 180]     # Breach: Semi-transparent Red
        status_text = "[FAILED] Metocean Limit Exceeded"

    # Precise coordinate parsing loop back into tabular format
    spatial_data = pd.DataFrame({
        "lat": [tag_meta["x"] / 10000 + 29.7604],
        "lon": [tag_meta["y"] / 10000 - 95.3698],
        "asset_tag": [selected_tag],
        "status": [status_text],
    })
    spatial_data["color"] = [hazard_color]

    # Render QGIS-style interactive 3D satellite twin engine using PyDeck
    st.pydeck_chart(pdk.Deck(
        map_style="mapbox://styles/mapbox/satellite-v9",
        initial_view_state=pdk.ViewState(
            latitude=spatial_data["lat"].iloc[0],
            longitude=spatial_data["lon"].iloc[0],
            zoom=17,
            pitch=50,
        ),
        layers=[
            pdk.Layer(
                ...
                get_elevation=12 if metocean_passed else 45,
                elevation_scale=1,
                radius=15,
