...
# ============================================================
# VODIDS | MRSIF-ROV — Tooling & Intervention | External Review Release
# ============================================================

st.set_page_config(
    page_title="VODIDS | MRSIF-ROV Tooling & Intervention",
    page_icon="🌊",
    layout="wide",
...
.external-review{
    ...
    background:#091621;border:1px solid #29465c;border-left:4px solid #33d1ff;
    border-radius:9px;padding:8px 10px;margin:6px 0 8px;
}
.external-main{font-size:8.8px;color:#bdd2dd;line-height:1.4}
.external-main b{color:#eef7fb}
.external-side{text-align:right;font-size:7.5px;color:#748d9d;line-height:1.4}
.module-tag{
    display:inline-block;border:1px solid #33d1ff;border-radius:12px;
    padding:3px 7px;color:#76dbff;font-size:7.5px;font-weight:800;margin-top:3px
}
...
gate_status = {
    ...
    "ROV Hydraulic Capability": stage_status(7,hydraulic_pass,6),
...
def build_downloadable_report(mission, gate_status):
    evidence = [
        ...
        "ROV hydraulic supply capability verified against the approved tool profile; valve function completed and independently verified where available.",
...
<h1>VODIDS · MRSIF™ | ROV Tooling & Intervention Mission Assurance Report</h1>
<div class="meta">
...
<tr><td>ROV Tool Hydraulic Supply Pressure</td><td>{st.session_state.hydraulic_psi} psi · source: ROV HPU / tool control</td></tr>
<tr><td>ROV Hydraulic Flow Capability</td><td>{st.session_state.hydraulic_flow_lpm:.1f} L/min · vehicle capability / configured engineering input</td></tr>
<tr><td>Tool Telemetry</td><td>NOT INSTRUMENTED / NOT AVAILABLE in this demonstration</td></tr>
<tr><td>Measured Tool Flow</td><td>NOT INSTRUMENTED — vehicle flow capability only is represented</td></tr>
<tr><td>Valve State</td><td>100% OPEN · mission outcome representation / independent confirmation required in real deployment</td></tr>
</table>
<p class="note">External-review demonstration using representative mission data. MRSIF records only evidence available from configured or connected systems and does not assume tool telemetry where no instrumented source exists. Production deployment shall use approved project requirements, OEM data and validated live interfaces. This demonstration does not imply OEM, CFIHOS or OSDU endorsement.</p>
</body></html>"""
...
<div class="topbar">
  <div class="brand">{logo_html()}
    <div>
      <div class="brand-title">MRSIF <span style="color:#33d1ff;">· ROV Tooling & Intervention</span></div>
      <div class="brand-sub">Marine Robotics & Subsea Intelligence Framework · Current Application Module: ROV Tooling / Intervention Mission Assurance</div>
    </div>
  </div>
  <div style="text-align:right">
    <div style="font-size:10px;color:{state_color};font-weight:800">{mission_state}</div>
    <div style="font-size:8px;color:#7890a1">DEMONSTRATION MODE · LIVE DATA INTERFACES NOT CONNECTED</div>
  </div>
</div>
""", unsafe_allow_html=True)
...
<div class="external-review">
  <div class="external-main">
    <b>MRSIF™ is the generic operational intelligence framework.</b>
    This screen demonstrates one application module: <b>ROV Tooling & Intervention</b>.
    MRSIF combines mission requirements, asset context, robotics/survey capability,
    engineering constraints, operational evidence and unbypassable execution gates
    according to the active application.
    <br><span class="module-tag">EXTERNAL REVIEW DEMONSTRATOR</span>
  </div>
  <div class="external-side">
    Representative mission data<br>
    Live offshore interfaces not connected<br>
    © 2026 VODIDS · All rights reserved
  </div>
</div>
""", unsafe_allow_html=True)
...
st.markdown(f"""
<div class="std-strip">
  <div class="std-card"><div class="std-k">Application Context · Asset Information</div><div class="std-v">CFIHOS</div><div class="std-n">Applied where asset/tag/property/handover context is required</div></div>
  <div class="std-card"><div class="std-k">Application Context · Energy Data</div><div class="std-v">OSDU®</div><div class="std-n">Applied where subsurface / spatial / survey data context is required</div></div>
  ...
  <div class="std-card"><div class="std-k">Subsea Tooling / Interfaces</div><div class="std-v">API RP 17H</div><div class="std-n">Tooling / intervention interface reference</div></div>
  <div class="std-card"><div class="std-k">ROV Interface Reference</div><div class="std-v">ISO 13628-8</div><div class="std-n">ROV interface reference for subsea production systems</div></div>
</div>
""", unsafe_allow_html=True)

st.markdown("""
<div class="evidence-principle">
<b>MRSIF evidence rule:</b> validate only what the deployed system can prove.
Geometry, localization, vehicle capability, interface compatibility and verified mission outcome are accepted as evidence.
MRSIF does not infer tool measurements that are unavailable from the deployed system; where no instrumented source exists, the record remains <b>NOT INSTRUMENTED / NOT AVAILABLE</b>.
</div>
""", unsafe_allow_html=True)
...
<div class="eng-strip">
  ...
  <div class="eng-card"><div class="eng-k">ROV Tool Hydraulic Supply</div><div class="eng-v">{st.session_state.hydraulic_psi} psi</div><div class="eng-n">Available vehicle supply · source: ROV HPU / tool control</div></div>
</div>
""", unsafe_allow_html=True)
...
with st.expander("Engineering Parameter Controls · Demonstration Inputs", expanded=False):
    ...
    with e4:
        ...
        st.session_state.hydraulic_flow_lpm = st.number_input("ROV hydraulic flow capability (L/min)",0.0,30.0,float(st.session_state.hydraulic_flow_lpm),0.5,key="top_flow")
...
if page == "MISSION":
    ...
    with twin_col:
        ...
        <div style="background:#06121c;border:1px solid #284156;border-radius:12px;padding:8px;font-family:Arial,sans-serif;">
          ...
          <svg viewBox="0 0 960 555" width="100%" role="img" aria-label="ROV hot stab intervention and localization simulation">
            ...
            <text x="185" y="510" fill="#8fa6b8" font-size="8">VEHICLE FLOW CAPABILITY</text><text x="185" y="528" fill="#eef7fb" font-size="13" font-weight="700">{st.session_state.hydraulic_flow_lpm if stage>=7 else 0:.1f} L/min</text>
    ...
    with assurance_col:
        ...
        for label in ["Metocean","Point Cloud","Localization","Distance","Tool Match","Clearance","Alignment","Visibility/FLS","Pilot Review","ROV Hydraulic Capability"]:
    ...
    st.markdown(f"""
    <div class="mission-hero">
      <div class="hero-main"><div class="hero-label">VODIDS MRSIF™ · ROV Tooling & Intervention Final Mission Report</div><div class="hero-value">{mission.get('mission_name')}</div></div>
      <div class="hero-item"><div class="hero-label">Work Ref</div><div class="hero-value">{mission.get('mission_id')}</div></div>
      <div class="hero-item"><div class="hero-label">Target</div><div class="hero-value">{mission.get('target_tag')}</div></div>
    ...
    with rr1:
        ...
        for k,v in [
            ...
            ("Tool Data","Geometry + interface + vehicle hydraulic capability"),
            ("Tool Telemetry","Not assumed unless instrumented"),
...
st.markdown(f'<div style="margin-top:9px;border-top:1px solid #1e3141;padding-top:7px;color:#6f8798;font-size:8px;display:flex;justify-content:space-between"><span>VODIDS · MRSIF™ | ROV Tooling & Intervention · External Review Release</span><span>Representative mission data · Live interfaces not connected · {ts}</span></div>', unsafe_allow_html=True)
